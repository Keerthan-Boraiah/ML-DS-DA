...
def crossover_child(parents, offspring_size):
    ...
    AUTHOR:- Myself
    REASON:- reason was to get only 5 chromosomes in the final. +1 is so the condition will break if the offspring is already the length of 5 because after the mutation 
    I'll get different length chromosomes. so if the length is not equal then after mutation and adding the 1 random value will be added to the gene, 
    and would give me always 5 chromosomes gene.
    """
...
random initial population let's say 2 genes, and a genetic algorithm to move up the gene value to different lenght like 3,4,5 to get a good fitness value. 
So, I went with only option whuch gets best with only the 5th gene which was giving a good score. 
